...
export default function Planner() {
  ...
  async function markComplete(task) {
    try {
      await plannerApi.updateTask(task.id, { status: 'completed', completed_hours: task.estimated_hours });
      loadTasks();
    } catch {}
  }
  ...
  async function deleteTask(id) {
    ...
    try { await plannerApi.deleteTask(id); loadTasks(); } catch {}
  }
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage your tasks and study goals.</p>
        </div>
      ...
      {error && <div className="mb-4 px-4 py-3 bg-red-900/40 border border-red-700 rounded-lg text-red-300 text-sm">{error}</div>}

      {/* Modal */}
      {showForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-lg">
            <h3 className="text-lg font-semibold text-white mb-6">Add New Task</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" value={form.estimatedHours} min="0.5" max="100" step="0.5"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {tasks.map(task => (
            <div key={task.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center gap-4">
              <div className={`w-2 h-12 rounded-full flex-shrink-0 ${
                task.status === 'completed' ? 'bg-emerald-500' : task.status === 'missed' ? 'bg-red-500' : 'bg-indigo-500'
              }`}/>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className={`text-white font-medium ${task.status === 'completed' ? 'line-through text-gray-500' : ''}`}>{task.title}</p>
                ...
                <div className="flex items-center gap-4 mt-1">
                  <p className="text-gray-500 text-xs">
                    ...
                    <span className={daysUntil(task.deadline) <= 3 ? ' text-red-400' : ' text-gray-500'}>
                      {' '}({daysUntil(task.deadline)} days left)
                    </span>
                  </p>
                  <p className="text-gray-500 text-xs">⏱ {task.completed_hours}/{task.estimated_hours} hrs</p>
                </div>
                <div className="mt-2 h-1 bg-gray-800 rounded-full w-48">
                  <div className="h-1 bg-indigo-500 rounded-full" style={{ width: `${Math.min((task.completed_hours / task.estimated_hours) * 100, 100)}%` }}/>
                </div>
              </div>
