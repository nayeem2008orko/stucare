...
export default function Dashboard() {
  ...
  const [availHours, setAvailHours] = useState(() => Number(localStorage.getItem('stucare_avail_hours') || 4));

  const user = JSON.parse(localStorage.getItem('stucare_user') || '{}');
  const hour = new Date().getHours();
  const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';

  useEffect(() => { loadAll(availHours); }, []);

  // Refresh plan when user navigates back to this page from Planner
  useEffect(() => {
    const onFocus = () => loadAll(availHours);
  ...
  async function loadAll(hours) {
    ...
    } catch { setError('Could not load dashboard. Make sure all services are running.'); }
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold" style={{ color: '#1a1a2e' }}>{greeting}, {user.name?.split(' ')[0]}! 👋</h2>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Completion Rate', value: `${progress?.completionRate ?? 0}%`, sub: null, bar: progress?.completionRate ?? 0 },
          { label: 'Tasks Done',      value: progress?.completedTasks ?? 0,       sub: `of ${progress?.totalTasks ?? 0} total` },
          { label: 'Hours Studied',   value: `${(progress?.totalHoursStudied ?? 0).toFixed(1)}`, sub: `of ${(progress?.totalHoursPlanned ?? 0).toFixed(1)} planned` },
          ...
          <div key={i} className="rounded-xl p-5 border" style={{ background: '#E5EEE4', borderColor: '#c8dcc6' }}>
            ...
            {stat.bar !== undefined && (
              <div className="mt-2 h-1.5 rounded-full" style={{ background: '#c8dcc6' }}>
                <div className="h-1.5 rounded-full" style={{ width: `${stat.bar}%`, background: '#87CEFA' }}></div>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="rounded-xl p-6 border" style={{ background: '#E5EEE4', borderColor: '#c8dcc6' }}>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: '#8a9a8a' }}>{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          </div>
          <div className="flex items-center gap-3">
            <label className="text-sm" style={{ color: '#6b7a6b' }}>Available hours:</label>
            <select value={availHours} onChange={e => handleHoursChange(Number(e.target.value))}
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-3 text-sm" style={{ color: '#6b7a6b' }}>Generating your personalized plan...</p>
          </div>
        ) : error ? (
          <div className="text-center py-8">
            <p className="text-sm" style={{ color: '#c0392b' }}>{error}</p>
            <button onClick={loadAll} className="mt-3 text-sm" style={{ color: '#3a9ad9' }}>Try again</button>
          </div>
        ) : !plan?.schedule?.length ? (
          <div className="text-center py-12">
            <p className="text-4xl mb-4">📚</p>
            <p className="font-medium" style={{ color: '#1a1a2e' }}>No tasks scheduled for today</p>
            <p className="text-sm mt-1" style={{ color: '#6b7a6b' }}>Add subjects in the Planner to generate a study plan</p>
            <Link to="/planner" className="inline-block mt-4 px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              style={{ background: '#87CEFA', color: '#1a1a2e' }}>
              Go to Planner
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {plan.schedule.map((item, i) => (
              <div key={i} className="flex items-center gap-4 p-4 rounded-xl border transition-colors"
                ...
                <div className="w-px h-10" style={{ background: '#c8dcc6' }}></div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={diffClass[item.difficulty]}>{item.difficulty}</span>
