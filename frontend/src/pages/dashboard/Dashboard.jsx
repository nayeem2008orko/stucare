          ...
          <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            {stat.bar !== undefined && (
              <div className="mt-2 h-1.5 bg-gray-800 rounded-full">
                <div className="h-1.5 bg-indigo-500 rounded-full" style={{ width: `${stat.bar}%` }}></div>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <select value={availHours} onChange={e => setAvailHours(Number(e.target.value))}
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 mt-3 text-sm">Generating your personalized plan...</p>
          </div>
        ) : error ? (
          <div className="text-center py-8">
            <p className="text-red-400 text-sm">{error}</p>
            <button onClick={loadAll} className="mt-3 text-indigo-400 hover:text-indigo-300 text-sm">Try again</button>
          </div>
        ) : !plan?.schedule?.length ? (
          <div className="text-center py-12">
            <p className="text-4xl mb-4">📚</p>
            <p className="text-gray-300 font-medium">No tasks scheduled for today</p>
            <p className="text-gray-500 text-sm mt-1">Add subjects in the Planner to generate a study plan</p>
            <Link to="/planner" className="inline-block mt-4 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm transition-colors">
              Go to Planner
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {plan.schedule.map((item, i) => (
              <div key={i} className="flex items-center gap-4 p-4 bg-gray-800/50 border border-gray-700/50 rounded-xl hover:border-gray-600 transition-colors">
                <div className="text-center min-w-[70px]">
                  <p className="text-indigo-400 font-semibold text-sm">{item.start_time}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-300 text-sm font-medium">{item.duration_min}m</span>
                </div>
              </div>
            ))}
          </div>
