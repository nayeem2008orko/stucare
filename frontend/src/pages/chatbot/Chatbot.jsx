...
function renderMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^#{1,3}\s/.test(line)) {
      ...
      const cls =
        level === 1 ? "text-base font-bold mt-2" : "text-sm font-bold mt-1";
      elements.push(
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`"))
      return (
        <code
          key={i}
          className="bg-gray-700 text-indigo-300 px-1 rounded text-xs"
        >
          {part.slice(1, -1)}
        </code>
      );
...
export default function Chatbot() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: res.data.data.reply, mode },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
    ...
    <div className="flex flex-col h-screen p-8 pb-0">
      <div className="mb-6 flex-shrink-0">
        ...
        <p className="text-gray-400 mt-1">
          Your personal study and motivation companion.
        </p>
      </div>
        ...
        {mode === "study" ? (
          <div className="flex items-center gap-2 px-3 py-2 bg-indigo-900/30 border border-indigo-800/50 rounded-lg">
            ...
            <p className="text-indigo-300 text-xs">
              Study mode — Ask questions, get explanations, solve problems
            </p>
          </div>
        ) : (
          <div className="flex items-center gap-2 px-3 py-2 bg-violet-900/30 border border-violet-800/50 rounded-lg">
            ...
            <p className="text-violet-300 text-xs">
              Motivation mode — Share how you feel, get encouragement and
              support
            </p>
          </div>
        )}
